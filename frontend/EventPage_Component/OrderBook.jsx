...
const OrderBookService = {
  ...
  async fetchOrderBookSummary(eventId) {
    const response = await fetch(
      `${API_BASE_URL}/orderbook/${eventId}/summary`
    );
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(
...
const OrderBook = ({ eventId = "demo" }) => {
  ...
  const setSampleData = () => {
    const sampleYesOrders = [
      { price: 6.5, yesQty: 923, yesOrderCount: 5 },
      { price: 7.0, yesQty: 1650, yesOrderCount: 8 },
      { price: 7.5, yesQty: 253, yesOrderCount: 3 },
      { price: 8.0, yesQty: 2, yesOrderCount: 1 },
      { price: 9.0, yesQty: 1, yesOrderCount: 1 },
    ];

    const sampleNoOrders = [
      { price: 4.0, noQty: 12, noOrderCount: 2 },
      { price: 4.5, noQty: 1219, noOrderCount: 12 },
      { price: 5.0, noQty: 6223, noOrderCount: 25 },
      { price: 5.5, noQty: 251, noOrderCount: 4 },
      { price: 6.0, noQty: 3, noOrderCount: 1 },
    ];
    ...
    const fetchOrderBook = async () => {
      ...
      try {
        ...
        if (yesResult.success) {
          ...
          throw new Error(
            yesResult.message || "Failed to fetch YES order book"
          );
        }
  ...
  const maxYesQty =
    yesOrders.length > 0
      ? Math.max(...yesOrders.map((item) => item.yesQty || 0))
      : 1;
  const maxNoQty =
    noOrders.length > 0
      ? Math.max(...noOrders.map((item) => item.noQty || 0))
      : 1;

  const getYesBarWidth = (qty) =>
    qty > 0 ? Math.min((qty / maxYesQty) * 100, 100) : 0;
  const getNoBarWidth = (qty) =>
    qty > 0 ? Math.min((qty / maxNoQty) * 100, 100) : 0;
  ...
  // Sort YES orders by quantity in descending order and take top 6
  const sortedYesOrders = [...yesOrders]
    .sort((a, b) => (b.yesQty || 0) - (a.yesQty || 0))
    .slice(0, 6);

  // Sort NO orders by quantity in descending order and take top 6
  const sortedNoOrders = [...noOrders]
    .sort((a, b) => (b.noQty || 0) - (a.noQty || 0))
    .slice(0, 6);
  ...
  const combinedOrderData = [];
  const maxLength = Math.max(sortedYesOrders.length, sortedNoOrders.length);
  for (let i = 0; i < maxLength; i++) {
    combinedOrderData.push({
      yesPrice: sortedYesOrders[i]?.price,
      yesQty: sortedYesOrders[i]?.yesQty,
      yesOrderCount: sortedYesOrders[i]?.yesOrderCount,
      noPrice: sortedNoOrders[i]?.price,
      noQty: sortedNoOrders[i]?.noQty,
      noOrderCount: sortedNoOrders[i]?.noOrderCount,
    });

    const maxYesQty =
      sortedYesOrders.length > 0
        ? Math.max(...sortedYesOrders.map((item) => item.yesQty || 0))
        : 1;
    const maxNoQty =
      sortedNoOrders.length > 0
        ? Math.max(...sortedNoOrders.map((item) => item.noQty || 0))
        : 1;
  }
              ...
              {combinedOrderData.length > 0 ? (
                <div className="space-y-2">
                  {combinedOrderData.map((row, index) => (
                    <div
                      key={index}
                      className="grid grid-cols-2 sm:grid-cols-6 gap-2 sm:gap-4 py-2 hover:bg-gray-50 rounded"
                    >
                      {/* Mobile YES View */}
                        ...
                        <div className="flex justify-between">
                          ...
                          <div className="relative w-3/4">
                            <div
                              ...
                              style={{
                                width: `${getYesBarWidth(row.yesQty)}%`,
                              }}
                            ></div>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-medium text-gray-900">
                      {summary.yes.totalQuantity.toLocaleString()}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-green-700">Order Count:</span>
                    <span className="font-medium text-gray-900">
                      {summary.yes.orderCount}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-green-700">Average Price:</span>
                    <span className="font-medium text-gray-900">
                      {formatPrice(summary.yes.averagePrice)}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-red-50 p-4 rounded-lg border border-red-200">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-medium text-gray-900">
                      {summary.no.totalQuantity.toLocaleString()}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-red-700">Order Count:</span>
                    <span className="font-medium text-gray-900">
                      {summary.no.orderCount}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-red-700">Average Price:</span>
                    <span className="font-medium text-gray-900">
                      {formatPrice(summary.no.averagePrice)}
                    </span>
                  </div>
                </div>
              </div>
